...
try:
    ...
    with tab_rekod:
        if df_jadual.empty:
            st.error(f"Fail '{NAMA_FAIL_PDF}' tidak ditemui atau ralat semasa membaca. Sila muat naik fail ke GitHub.")
        else:
            ...
            with col_side2:
                ...
                if jadual_guru.empty:
                    st.info(f"Tiada kelas untuk {pilihan_guru} pada hari {hari_pilihan}.")
                else:
